...
### 1.1 SS-MIX2 Ver.1.2g機能

SS-MIX2 Ver.1.2gに準拠することとして、以下の機能を有すること。
 
* 日本医療情報学会発行の「SS-MIX2標準化ストレージ構成の説明と構築ガイドラインVer.1.2g」、「SS-MIX2拡張ストレージ構成の説明と構築ガイドラインVer.1.2g」、「SS-MIX2標準化ストレージ仕様書Ver.1.2g」、「標準化ストレージ仕様書別紙：コード表Ver.1.2g」、「SS-MIX2拡張ストレージ構成の説明と構築ガイドラインVer.1.2g別紙：標準文書コード表」に記載している仕様に対応していること。（尚、当初Ver.1.2c準拠としていたが、全社改修を行ったためVer.1.2g準拠ということとした。）
...
　「SS-MIX2標準化ストレージ構成の説明と構築ガイドラインVer.1.2g　p11」
　
### 1.2 拡張ストレージへの出力機能
...
## 1.3　NHO対応としての設定
### 1.3.0 拡張ストレージへの出力機能
各社のSS-MIX2モジュールの拡張ストレージへの出力機能を利用し、以下の情報を出力すること。その際、 日本医療情報学会発行の「SS-MIX2拡張ストレージ構成の説明と構築ガイドラインVer.1.2g」に記載している仕様に対応していること。また、トランザクションストレージ、インデックスデータベースも同時に生成すること。
...
（１）バイタルデータとして取得する項目は、「拡張期血圧、収縮期血圧、脈拍数、呼吸数、体温、経皮的動脈血酸素飽和度(SpO2)」の６項目とする。
...
### 1.3.11 ICD-10のバージョン
「JAHIS病名情報データ交換規約Ver.3.1C」P8では、**PRB-10 の第7 成分の値が従来通りnull であるか「2003」をセットした場合は2003 年版、「2013」をセットした場合は2013 年版のコードとみなすこととする**と定められている。